...
![](images\gendouyun01.png)

当鼠标移动到下一个菜单的时候，背景的效果会 慢慢移动过去 。

![](images\gendouyun02.png)
...
最后添加的` e.preventDefault();`到底是为什么？啥意思呢？

现在我们先给阻止默认行为的代码去掉，然后将body的高度设置为2000px（只要能够可以上下滚动即可），然后看下效果，效果如下：

![](images\移动端拖拽元素不阻止默认行为效果.gif)

因为默认上下拖拽是可以拖动屏幕滚动的，如果div拖拽不阻止默认屏幕滚动行为，就会出现如上效果。